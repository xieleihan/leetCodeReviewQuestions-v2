...
进制陷阱  map回调是`callback(currentValue, index, array)`

`parseInt(string, radix)`注意：`parseInt` 接受两个参数，第一个是要解析的字符串，第二个是进制。
...
0   // 第 1 次 log （同步代码区）
0   // 第 2 次 log （同步代码区）
1   // 第 3 次 log （setTimeout 回调）
2   // 第 4 次 log （setTimeout 回调）
```

# ❗ 为什么前两个 log 都是 0？

因为在 React（特别是 class 组件）中：

### ➤ 在生命周期（如 componentDidMount）中

`setState()` 是 **异步 & 批处理（batched）** 的。

所以在同步流程里：

```
this.setState({ val: this.state.val + 1 }); // 请求更新 val = 1
console.log(this.state.val); // 还是 0

this.setState({ val: this.state.val + 1 }); // 再次请求更新 val = 1
console.log(this.state.val); // 还是 0
```

为什么第二次也还是更新到 1？
 因为你使用的是 **对象式 setState**，它依赖旧的 state：

- 第一次 setState 看到的有 `this.state.val = 0`
- 第二次 setState 看到的仍然是 `this.state.val = 0`
...
### ➤ setTimeout 回调 → **不在 React 批处理范围内**

在旧版 React（<=18 默认模式）里：

- 生命周期、事件处理 → 批处理（异步）
- 原生 async 回调（如 setTimeout） → **非批处理（同步执行 setState）**
...
此时 React 已经执行完前面的两次 setState，
 此时 `val = 1`。

上面的 setState 在 setTimeout 里是 **同步** 的，所以立即更新：
...
注意：这里 log 的值是 **旧值** 1，因为对象式 setState 依然看到旧 state。
...
现在 this.state.val 是 2，所以：

```
val = 2 + 1 = 3
console.log(this.state.val) // 输出旧值：2
```
...
## 1️⃣ 同步代码里：setState 异步 + 批处理

生命周期、事件处理等 React 控制的同步代码中：
...
## 2️⃣ setTimeout 中：setState 是同步的（React18 旧模式）

- 不会批处理
- 每次 setState 都会立即更新 state
- this.state 是马上变的

------

## 3️⃣ ❗ 对象式 setState 会“锁住旧值”

```
this.setState({ val: this.state.val + 1 })
```

永远用的是调用 setState 那一刻的旧 state。

要避免这种 bug，应该用 **函数式 setState**：
...
> 这行代码是关键： `a.x = a = { n: 2 };`
>
> 在 JavaScript 中，**点属性访问 (`.`)** 的优先级高于 **赋值运算符 (`=`)**。并且，赋值运算符是从**右向左**结合的
>
> ### 逐步分析
>
> 1. **优先级处理：** 尽管赋值是从右向左执行，但在执行任何赋值操作之前，会先计算左侧的表达式。
>    - **左侧的表达式是 `a.x`。** 此时的 `a` 仍然指向最初的对象 `{ n: 1 }`。JavaScript **锁定了**这个操作：它将要设置 **初始 `a` 所指向的那个对象的 `x` 属性**。
>    - **记住：** `a` 和 `b` 目前都指向同一个对象（称它为 **Object_1**: `{ n: 1 }`）。
> 2. **赋值执行（从右向左）：**
>    - **右侧赋值：** `a = { n: 2 }`
>      - 执行这个操作后，**变量 `a`** 被赋予了一个全新的对象（称它为 **Object_2**: `{ n: 2 }`）。
>      - 此时，`a` 指向 Object_2，而 `b` 仍然指向 Object_1。
>    - **左侧赋值：** `a.x = ...`（实际上是 **Object_1.x** 的赋值）
>      - 由于第一步锁定了操作目标是 **初始 `a` 所指向的对象** 的 `x` 属性 (即 **Object_1.x**)，并将整个赋值表达式（即 `a = { n: 2 }` 的结果，也就是 **Object_2**: `{ n: 2 }`）赋给它。
>      - 所以，**Object_1** 现在变成了 `{ n: 1, x: { n: 2 } }`
