...
## (二、网络篇-1-webpack-性能调优与-gzip-原理)二、网络篇 1：webpack 性能调优与 Gzip 原理
...
## (三、网络篇-2-图片优化-质量与性能的博弈)三、网络篇 2：图片优化——质量与性能的博弈
...
## (四、存储篇-1-浏览器缓存机制介绍与缓存策略剖析)四、存储篇 1：浏览器缓存机制介绍与缓存策略剖析
...
## (五、存储篇-2-本地存储-从-cookie-到-web-storage、indexdb)五、存储篇 2：本地存储——从 Cookie 到 Web Storage、IndexDB
...
### (故事的开始-从-cookie-说起)故事的开始：从 Cookie 说起
...
### (向前一步-web-storage)向前一步：Web Storage
...
## (六、cdn-的缓存与回源机制解析)六、CDN 的缓存与回源机制解析

------

### (cdn的缓存与回源机制解析)CDN的缓存与回源机制解析
...
## (七、渲染篇-1-服务端渲染的探索与实践)七、渲染篇 1：服务端渲染的探索与实践
...
## (八、渲染篇-2-知己知彼-解锁浏览器背后的运行机制)八、渲染篇 2：知己知彼——解锁浏览器背后的运行机制
...
## (不做无用功-基于渲染流程的-css-优化建议)不做无用功：基于渲染流程的 CSS 优化建议
...
### (告别阻塞-css-与-js-的加载顺序优化)告别阻塞：CSS 与 JS 的加载顺序优化
...
## (九、渲染篇-3-对症下药-dom-优化原理与基本实践)九、渲染篇 3：对症下药——DOM 优化原理与基本实践
...
### (望闻问切-dom-为什么这么慢)望闻问切：DOM 为什么这么慢
...
### (药到病除-给你的-dom-提提速)药到病除：给你的 DOM “提提速”
...
## (十、渲染篇-4-千方百计-event-loop-与异步更新策略)十、渲染篇 4：千方百计——Event Loop 与异步更新策略
...
### (前置知识-event-loop-中的-渲染时机)前置知识：Event Loop 中的“渲染时机”

搞懂 `Event Loop`，是理解 `Vue` 对 `DOM` 操作优化的第一步。

### (micro-task-与-macro-task)Micro-Task 与 Macro-Task

- 事件循环中的异步队列有两种：`macro`（宏任务）队列和 `micro`（微任务）队列。
- 常见的 `macro-task` 比如： `setTimeout`、`setInterval`、 `setImmediate`、`script`（整体代码）、 `I/O` 操作、`UI` 渲染等。
- 常见的 `micro-task` 比如: `process.nextTick`、`Promise`、`MutationObserver` 等。

### (event-loop-过程解析)Event Loop 过程解析
...
### (生产实践-异步更新策略-以-vue-为例)生产实践：异步更新策略——以 Vue 为例
...
## (十一、渲染篇-5-最后一击-回流-reflow-与重绘-repaint)十一、渲染篇 5：最后一击——回流（Reflow）与重绘（Repaint）
...
### (flush-队列-浏览器并没有那么简单)Flush 队列：浏览器并没有那么简单
...
## (十二、应用篇-1-优化首屏体验-lazy-load-初探)十二、应用篇 1：优化首屏体验——Lazy-Load 初探
...
### (一起写一个-lazy-load-吧)一起写一个 Lazy-Load 吧！
...
## (十三、应用篇-2-事件的节流-throttle-与防抖-debounce)十三、应用篇 2：事件的节流（throttle）与防抖（debounce）
...
### (节流-与-防抖-的本质)“节流”与“防抖”的本质

这两个东西都以**闭包**的形式存在。

它们通过对事件对应的回调函数进行包裹、以自由变量的形式缓存时间信息，最后用 setTimeout 来控制事件的触发频率。

### (throttle-第一个人说了算)Throttle： 第一个人说了算
...
### (debounce-最后一个人说了算)Debounce： 最后一个人说了算
...
### (用-throttle-来优化-debounce)用 Throttle 来优化 Debounce
...
## (十四、性能监测篇-performance、lighthouse-与性能-api)十四、性能监测篇：Performance、LightHouse 与性能 API
...
### (可视化监测-从-performance-面板说起)可视化监测：从 Performance 面板说起
...
### (可视化监测-更加聪明的-lighthouse)可视化监测： 更加聪明的 LightHouse
...
### (可编程的性能上报方案-w3c-性能-api)可编程的性能上报方案： W3C 性能 API
